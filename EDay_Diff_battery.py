...
ta_list = [900, 1800, 3600, 7200, 10800, 14400, 21600, 43200,
...
for k in range(0,len(Resolution),2):
    ...
    for j in range (len(sa_list)):
        for i in range(len(ta_list)):
            # print('sa', sa_list[i])
            MaxDataPacketSize = 1500  # (BLE)#(NBIOIT)#245(BLE/LoRa)  # Maximum size of a data packet in bytes
            PTx = 2.052  # Power consumption
            PRx = 0.1178  # as iactive# Power consumption
            PIdle = 0.04218  # asidle Power consumption
            PSleep = 0.00532    # Power consumption
            tsRx, tsTx, tsIdle = LT1_calculate_tx_time(sa_list[j])
            total_time = Lifetime_cal(ta_list[i], sa_list[j], MaxDataPacketSize, PTx, PRx, PIdle, PSleep, PER, tsTx, tsRx,
                                      tsIdle, Ebattery)
            total_time_inyear_LTECAT1 = total_time / (3600 * 24*365)
            total_time_inyear_list_LTECAT1.append(total_time_inyear_LTECAT1)
            #total_time_with_Solar_panel = Lifetime_cal_with_solarpanel(ta_list[i], sa_list[j], MaxDataPacketSize, PTx, PRx, PIdle, PSleep, PER, tsTx, tsRx, tsIdle, Ebattery, irradiance, panel_area, sunlight_hours, efficiency)
            #total_time_inyear_list_LTCAT4_with_solar_panel.append(total_time_with_Solar_panel / (3600 * 24*365))
        #plt.plot(np.array(ta_list) / 3600, total_time_inyear_list_LTECAT1, label=f"L.C.1_sa_{j}", marker='d', linestyle='-')
        plt.plot((24/(np.array(ta_list) / 3600))[::-1], total_time_inyear_list_LTECAT1[::-1], label=f"L.C.1_{Label[j]}", marker='d', linestyle=f'{Linestyle[j]}',  color=color[5])
        #plt.plot(np.array(ta_list) / 3600, total_time_inyear_list_LTCAT4_with_solar_panel, label=f"Solar_L.C.4_sa_{j}", marker='d', linestyle='-')
        total_time_inyear_list_LTECAT1.clear()
       # total_time_inyear_list_LTCAT4_with_solar_panel.clear()
    ...
    #plt.plot(np.array(sa_list) / 1000, total_time_inyear_list_BLE_with_solar_panel, label="BLE_solarpanel", marker='o',
            # linestyle='-')'''
    hline1=plt.hlines(Dcell_Eperday,xmin=0,xmax=100, label ="Dcell Energy_per_day",linestyles=f'{Linestyle[1]}', color=color[0])
    hline2=plt.hlines(AA_Eperday, xmin=0, xmax=100, label="AA Energy_per_day", linestyles=f'{Linestyle[2]}',
               color=color[7])
    hline3=plt.hlines(AAA_Eperday, xmin=0, xmax=100, label="AAA Energy_per_day", linestyles=f'{Linestyle[3]}',
               color=color[8])
    hline4=plt.hlines(CR2032_Eperday, xmin=0, xmax=100, label="CR2032 Energy_per_day", linestyles=f'{Linestyle[1]}',
               color=color[9])
    plt.yscale('log')
    plt.xlim(left=0)
    plt.xlabel(
    ...
    other_legend= plt.legend(handles=[handles[0],handles[1],handles[2],handles[3], handles[4]],loc='lower right', bbox_to_anchor=(1, 0.01), fontsize=6)
    battery_legend=plt.legend(handles=[hline1,hline2,hline3,hline4],loc='lower center', bbox_to_anchor=(0.5,0.01),fontsize=6)
