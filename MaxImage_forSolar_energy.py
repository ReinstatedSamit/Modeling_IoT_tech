...
Tech=['BLE','NBIOT','LoRa','LTEC4','WiFi_ah']
...
for k in range(0,len(Resolution),2):
    ...
    for j in range(k,k+1):
        ...
        ta_min=tsRx+tsTx+1 #capture and sending (0.5-1)
        consumed_energy_BLE=(ComputeEnergyToSendData_v3(sa_list[j], MaxDataPacketSize, PTx, PRx, PIdle, PSleep,
                                                                PER, tsTx, tsRx,
                                                                tsIdle, ta_min))

        total_energy = (consumed_energy_BLE) + 0.5 + ta_min * 3.3 * 7e-3
        Number_of_images_e=Outdoor_solar/total_energy
        Number_of_images_t=86400/ta_min
        Number_of_images=min(filter(lambda x: x>0,[Number_of_images_e, Number_of_images_t]))
        Transmission_interval=((86400-Number_of_images*ta_min)/Number_of_images)+ta_min
        #total_energy = (consumed_energy_BLE) + 0.5 + ta_min * 3.3 * 7e-3 + (86400 - Number_of_images * ta_min)
        #print(Number_of_images_e)
        #print(Number_of_images_t)
        print("-------------BLE---------------")
        #print(f"Resolution:{Resolution[j]}")
        print(f"BLE Number_of_images: {Number_of_images}")
        print(f"Images are transmitted every {Transmission_interval/60} minutes")
        BLE_plot=plt.scatter( Transmission_interval/60, Number_of_images, #label=f"BLE_{Label[j-k]}",
                 marker=f'{markers[j]}', linestyle=f'{Linestyle[j-k]}', color=color[1],s=150)
        data.append(['BLE', Resolution[k], Number_of_images, Transmission_interval / 60])
    ...
    for j in range(k,k+1):
        ...
        ta_min = tsIdle + tsRx + tsTx + 1  # capture and sending (0.5-1)
        consumed_energy_NBIOT=(ComputeEnergyToSendData_v3(sa_list[j], MaxDataPacketSize, PTx, PRx, PIdle, PSleep,
                                                                PER, tsTx, tsRx,
                                                                tsIdle, ta_min))

        total_energy = (consumed_energy_NBIOT) + 0.5 + ta_min * 3.3 * 7e-3
        Number_of_images_e = Outdoor_solar / total_energy
        Number_of_images_t = 86400 / ta_min
        Number_of_images = min(filter(lambda x: x > 0, [Number_of_images_e, Number_of_images_t]))
        Transmission_interval = ((86400 - Number_of_images * ta_min) / Number_of_images) + ta_min
        ...
        print("-------------NBIOT---------------")
        #print(f"Resolution:{Resolution[j]}")
        print(f"NBIOT Number_of_images: {Number_of_images}")
        print(f"Images are transmitted every {Transmission_interval/60} minutes")
        NBIOT_plot = plt.scatter( Transmission_interval/60, Number_of_images,
                              #label=f"NBIOT_{Label[j - k]}",
                               marker=f'{markers[j]}', linestyle=f'{Linestyle[j - k]}', color=color[2],s=150)
        data.append(['NBIOT', Resolution[k], Number_of_images, Transmission_interval / 60])
    ...
    for j in range(k,k+1):
        ...
        ta_min = tsIdle + tsRx + tsTx + 1  # capture and sending (0.5-1)
        consumed_energy_loRa = (ComputeEnergyToSendData_v3(sa_list[j], MaxDataPacketSize, PTx, PRx, PIdle, PSleep,
                                                           PER, tsTx, tsRx,
                                                           tsIdle, ta_min))

        total_energy = (consumed_energy_loRa) + 0.5 + ta_min * 3.3 * 7e-3
        Number_of_images_e = Outdoor_solar / total_energy
        Number_of_images_t = 86400 / ta_min
        Number_of_images = min(filter(lambda x: x > 0, [Number_of_images_e, Number_of_images_t]))
        Transmission_interval = ((86400 - Number_of_images * ta_min) / Number_of_images) + ta_min
        #total_energy = (consumed_energy_BLE) + 0.5 + ta_min * 3.3 * 7e-3 + (86400 - Number_of_images * ta_min)
        #print(consumed_energy_loRa)
        #print(Number_of_images_e)
        #print(Number_of_images_t)
        print("-------------LORA---------------")
        #print(f"Resolution:{Resolution[j]}")
        print(f"LoRa Number_of_images: {Number_of_images}")
        print(f"Images are transmitted every {Transmission_interval/60} minutes")
        Lora_plot = plt.scatter( Transmission_interval/60, Number_of_images,
                             #label=f"LORA_{Label[j - k]}",
                              marker=f'{markers[j]}', linestyle=f'{Linestyle[j - k]}', color=color[3],s=150)
        data.append(['LORA', Resolution[k], Number_of_images, Transmission_interval / 60])
    ...
    for j in range(k,k+1):
        ...
        ta_min = tsIdle + tsRx + tsTx + 1  # capture and sending (0.5-1)
        consumed_energy_LTECAT4 = (ComputeEnergyToSendData_v3(sa_list[j], MaxDataPacketSize, PTx, PRx, PIdle, PSleep,
                                                              PER, tsTx, tsRx,
                                                              tsIdle, ta_min))

        total_energy = (consumed_energy_LTECAT4) + 0.5 + ta_min * 3.3 * 7e-3
        Number_of_images_e = Outdoor_solar / total_energy
        Number_of_images_t = 86400 / ta_min
        Number_of_images = min(filter(lambda x: x > 0, [Number_of_images_e, Number_of_images_t]))
        Transmission_interval = ((86400 - Number_of_images * ta_min) / Number_of_images) + ta_min
        ...
        print("-------------LTEC4---------------")
        #print(f"Resolution:{Resolution[j]}")
        print(f"LTECAT4 Number_of_images: {Number_of_images}")
        print(f"Images are transmitted every {Transmission_interval/60} minutes")
      #  LTECAT4_plot = plt.scatter(Transmission_interval/60, Number_of_images,
                                #label=f"L.C.4_{Label[j - k]}",
                               # marker=f'{markers[j]}', linestyle=f'{Linestyle[j - k]}', color=color[4],s=150)
        data.append(['LTEC4', Resolution[k], Number_of_images, Transmission_interval / 60])
    ...
    for j in range(k,k+1):
        ...
        tsIdle = 0  # Idle time in seconds counted in Rx and Tx
        ta_min = tsIdle + tsRx + tsTx + 1  # capture and sending (0.5-1)
        consumed_energy_WiFiah=( ComputeEnergyToSendData_v3(sa_list[j], MaxDataPacketSize, PTx, PRx, PIdle, PSleep,
                                                            PER, tsTx, tsRx,
                                                            tsIdle, ta_min))

        total_energy = (consumed_energy_WiFiah) + 0.5 + ta_min * 3.3 * 7e-3
        Number_of_images_e = Outdoor_solar / total_energy
        Number_of_images_t = 86400 / ta_min
        Number_of_images = min(filter(lambda x: x > 0, [Number_of_images_e, Number_of_images_t]))
        Transmission_interval = ((86400 - Number_of_images * ta_min) / Number_of_images) + ta_min
        ...
        print("-------------WiFi_ah---------------")
        #print(f"Resolution:{Resolution[j]}")
        print(f"WiFi_ah Number_of_images: {Number_of_images}")
        print(f"Images are transmitted every {Transmission_interval/60} minutes")
       # WiFiah_plot = plt.scatter( Transmission_interval/60, Number_of_images,
                              # label=f"WiFiah_{Label[j - k]}",
                               # marker=f'{markers[j]}', linestyle=f'{Linestyle[j - k]}', color=color[5],s=150)
        data.append(['WiFi_ah', Resolution[k], Number_of_images, Transmission_interval / 60])
    ...
    #           color=color[9])
    hline5=plt.hlines(Outdoor_solar, xmin=0, xmax=1000, label="Outdoor_Solar", linestyles=f'{Linestyle[1]}',
               color=color[10])
    hline6=plt.hlines(Indoor_solar, xmin=0, xmax=1000, label="Indoor_solar", linestyles=f'{Linestyle[1]}',
               color=color[11])
    '''
plt.xscale('log')
#plt.yscale('log')
# plt.xlim(left=0)
plt.xlabel(
    f"Transmission Interval in minutes")  # ('Application time interval/Periodicity of image transfer each day (Hour)')
plt.ylabel('Number of Imager per day')
plt.title(f"Images Per Day for Different IoT Technologies using 53mmx35mm Solar panel for 3 hours at 40klx",
          fontsize=8, color='black', loc='left')
#print(Resolution[k])
# plt.legend(handles=[hline1,hline2,hline3,hline4],loc='lower center', bbox_to_anchor=(0.5,0.01),fontsize=6)
# Creating custom legend handles for communication technologies
tech_handles = []
for tech in range(0,3):
    tech_handles.append(Line2D([0], [0], color=f'{color[tech+1]}', markerfacecolor=color[tech], markersize=9, label=f'{Tech[tech]}'))


resolution_handles = []
for res in range(0,8,2):
    resolution_handles.append(Line2D([0], [0], marker=f'{markers[res]}' , color='b', markerfacecolor=color[tech], markersize=9, label=f'{Resolution[res]}'))

# Creating custom legend handles for references
#ref_handles = []
#for i, ref in enumerate(references):
#    ref_handles.append(Line2D([0], [0], marker=markers[i], color='w', markerfacecolor='black', markersize=10, label=f"{i+1}. {ref}"))

# Adding the legends to the plot
tech_legend=plt.legend(handles=tech_handles, loc='upper right', title='Communication Technology')
plt.gca().add_artist(tech_legend)

res_legend=plt.legend(handles=resolution_handles, loc='upper left', title='Resolution', bbox_to_anchor=(0.3, 1))
...
with open('communication_data.csv', 'w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(["Technology", "Resolution", "Number of Images", "Transmission Interval (minutes)"])
